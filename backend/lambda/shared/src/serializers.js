...
exports.serializeRegistration = serializeRegistration;
exports.serializeMessage = serializeMessage;
const portal_config_1 = require("./portal-config");
function buildProfileId(userId, role) {
...
function serializeUser(user) {
    const portal = (0, portal_config_1.getPortalConfig)(user.role);
    return {
        id: user.id,
...
function serializeEvent(event) {
    ...
    const myPortalBadge = myRegistrationStatus === "attended"
        ? "Attended"
    ...
    return {
        ...
        seatsRemaining: Math.max(0, Number(event.max_capacity ?? 0) - Number(event.current_count ?? 0)),
...
function serializeRegistration(registration) {
    ...
    const portalBadge = status === "attended"
        ? "Attended"
        : status === "cancelled"
            ? "Cancelled"
            : status === "waitlisted"
